	...
	return (
		<div className="fixed left-0 top-0 md:w-[230px] w-[60px] overflow-hidden h-full flex flex-col">
			...
			<span className="text-orange-400 font-semibold text-2xl md:block hidden">
  			<img src="src/assets/images/logo.jpeg" alt="Logo" width="250" height="" className="inline-block mr-2" />
			</span>
			...
			<div className="w-full h-[calc(100vh-70px)] border-r flex flex-col md:items-start items-center gap-2 border-slate-300 bg-[#fff] py-5 md:px-3 px-3 relative">
				{navLinks.map((link) => {
					return (
						<div
							key={link.title}
							className={`flex items-center gap-2 w-full rounded-lg hover:bg-white-400 px-2 py-3 cursor-pointer ${
								link.active ? "bg-[#e25647]" : "bg-transparent"
							}`}
						>
							{link.icon}
